...
export default async function handler(req: NextRequest) {
  ...
  const translationsWithIds = translations.data.translations.map((t, idx) => {
    ...
    // ImgIx needs base64 encoded strings, but btoa() isn't reliable for non-english chars; so use TextEncoder first
    const enconder = new TextEncoder()
    const endededQuote = enconder.encode(quote)
    const encodedU8Arr = Array.from(endededQuote)
